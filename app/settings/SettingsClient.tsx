...
// Client form for the settings page. Receives initial Firestore-backed values
// from the Server Component shell; saves via the saveSettings Server Action.
...
export default function SettingsClient({ initial }: { initial: SettingsInitialData }) {
  const { user, signOut } = useAuth();
  const router = useRouter();

  // Form state seeded from server-fetched Firestore values
  ...
  const [notifications, setNotifications] = useState<NotificationSettings>(initial.notificationPrefs);
  ...
  async function handleSaveProfile() {
    ...
    try {
      // Update Firebase Auth display name client-side (the source of truth for
      // the rendered Navbar identity). Best-effort — server save still proceeds.
      try {
        ...
        console.debug("settings: skipped Firebase Auth profile update", err);
      }
